...
const VirtualNumberOrder = sequelize.define('VirtualNumberOrder', {
  ...
  userId: { type: DataTypes.BIGINT, allowNull: false },
  // Legacy rows predate multi-provider support and are migrated to SMSBower.
  providerId: { type: DataTypes.STRING(24), allowNull: false, defaultValue: 'smsbower' },
  ...
  profitUsd: { type: DataTypes.DECIMAL(18, 8), allowNull: false, defaultValue: 0.15 },
...
async function initializeDatabase() {
  ...
  await addColumnIfMissing('VirtualNumberOrders', 'providerId', { type: DataTypes.STRING(24), allowNull: false, defaultValue: 'smsbower' });
  // Add profit as nullable first so historical rows can retain their actual
  // sale margin instead of being mislabeled with the new $0.15 default.
  await addColumnIfMissing('VirtualNumberOrders', 'profitUsd', { type: DataTypes.DECIMAL(18, 8), allowNull: true });
  await sequelize.query(`
    UPDATE ${tableSql('VirtualNumberOrders')}
    SET "profitUsd" = GREATEST(
      COALESCE("salePriceUsd", 0)::numeric - COALESCE("providerCostUsd", 0)::numeric,
      0
    )
    WHERE "profitUsd" IS NULL
  `);
  await sequelize.query(`
    ALTER TABLE ${tableSql('VirtualNumberOrders')}
    ALTER COLUMN "profitUsd" SET DEFAULT 0.15,
    ALTER COLUMN "profitUsd" SET NOT NULL
  `);
  ...
  // own API, so SMSBower and SMS-MAN may legitimately return the same id.
  await sequelize.query(`
    DROP INDEX IF EXISTS ${quoteIdent(config.databaseSchema)}."virtual_number_orders_activation_id"
  `);
  await sequelize.query(`
    CREATE INDEX IF NOT EXISTS "vno_provider_status_created_idx"
    ON ${tableSql('VirtualNumberOrders')} ("providerId", "status", "createdAt")
  `);
  await sequelize.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS "vno_provider_activation_uq"
    ON ${tableSql('VirtualNumberOrders')} ("providerId", "activationId")
    WHERE "activationId" IS NOT NULL
  `);
  await sequelize.query(`
