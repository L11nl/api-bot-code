...
const NetworkSettlement = sequelize.define('NetworkSettlement', {
  ...
  indexes: [
    ...
    { unique: true, fields: ['debtorShopId', 'sourceMethod', 'sourceRef'] }
  ]
});
...
async function migrateLegacySingleFileBotData() {
  ...
  for (const product of products) {
    ...
    if (
      ...
      ['photo', 'video'].includes(
        String(description.type || '').toLowerCase()
      ) &&
      description.fileId &&
      !product.image
    ) {
  ...
  try {
    ...
    for (const section of sections || []) {
      ...
      await Merchant.update(
        ...
        {
          where: {
            category: `digital_section_${section.id}`
          }
        }
      );
    }
  ...
  try {
    const existingNumber = await Setting.findOne({
      where: {
        key: 'superqi_number',
        lang: 'global'
      }
    ...
    const existingRate = await Setting.findOne({
      where: {
        key: 'iqd_rate',
        lang: 'global'
      }
    ...
    if (!existingNumber || !existingRate) {
      ...
      if (legacy) {
        ...
        const methodNumber = Array.isArray(methods)
          ? String(
              methods.find(item => item && item.value)?.value || ''
            ).trim()
          : '';
        ...
        const walletLooksPlaceholder =
          !walletNumber ||
          /superkey|\.\.\.|123456/i.test(walletNumber);

        const superQiNumber =
          !walletLooksPlaceholder
            ? walletNumber
            : methodNumber;
    ...
    for (const row of rows) {
      ...
      if (
        ...
        if (isAvailableUnused) {
          ...
          row.isUsed = true;
          row.maxUses = Math.max(
            1,
            Number(row.usedCount || 1)
          );

          await row.save({
            fields: ['isUsed', 'maxUses']
          });
        }
...
async function initializeDatabase() {
  console.log(
    `[DB] role=${config.network.role} schema=${config.databaseSchema}`
  );

  await sequelize.authenticate();

  await sequelize.query(
    `CREATE SCHEMA IF NOT EXISTS ${quoteIdent(config.databaseSchema)}`
  );
  ...
  /*
   * مهم جداً:
   * هذه الأعمدة يجب إضافتها قبل إنشاء الـ Indexes.
   * هذا هو إصلاح مشكلة Railway:
   *
  ...
  /*
   * الـ Indexes تأتي هنا بعد إضافة كل الأعمدة.
   */
  ...
  await sequelize.query(`
    ...
    ON ${tableSql('Codes')}
    ("merchantId", "stockOwnerShopId", "isUsed")
  `);
  ...
  await sequelize.query(`
    ...
    ON ${tableSql('NetworkDebtPayments')}
    ("creditorShopId", "status", "submittedOrderId")
  `);
    ...
    SET
      ...
      "usedCount" =
        CASE
          WHEN
            "isUsed" = TRUE
            AND COALESCE("usedCount", 0) = 0
          THEN 1
          ELSE COALESCE("usedCount", 0)
        END,
      "buyers" = COALESCE(
        "buyers",
        '[]'::jsonb
  ...
  await sequelize.query(`
    UPDATE ${tableSql('Codes')}
    SET "stockOwnerShopId" =
      COALESCE(
        NULLIF("stockOwnerShopId", ''),
        'master'
      )
  `).catch(() => {});
  ...
  await sequelize.query(`
    ...
    SET "contributionPriceUsd" =
      COALESCE(
        c."contributionPriceUsd",
        m."price"
      )
...
async function getSuperQiNumber() {
  return getSetting(
    'superqi_number',
    config.superQiNumber
  );
}

async function getSecureSetting(
  key,
  fallback = ''
) {
  const row =
    await SecureSetting.findByPk(key);
...
async function setSecureSetting(
  key,
  value
) {
  const encrypted =
  ...
  const [row] =
    await SecureSetting.findOrCreate({
      where: {
        key
      },
      defaults: {
